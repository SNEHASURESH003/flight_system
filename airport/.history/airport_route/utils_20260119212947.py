...
def shortest_between_airports(a1,a2):
    """
    Get the shortest route between two airports.

    :param a1: Airport object representing the starting airport
    :param a2: Airport object representing the destination airport
    :return: The Route object with the shortest duration between the two airports or None if no routes exist
    """
    routes = [route for route in a1.routes_from.all() if route.airport_code_to == a2]
    if not routes:
        return None
